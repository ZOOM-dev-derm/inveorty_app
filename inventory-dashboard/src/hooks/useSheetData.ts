...
export function useAddOrder() {
  const client = useQueryClient();
  return useMutation({
    mutationFn: (data: { orderDate: string; supplierSku: string; dermaSku: string; quantity: string; productName: string; productionDate: string; expectedDate: string; log?: string; container?: string; distributionNotes?: string; packagingLabels?: string; formula?: string; content?: string }) => addOrder(data),
    onSuccess: () => {
      client.invalidateQueries({ queryKey: ["orders"] });
    },
  });
